...
namespace csDronLink2
{
    ...
    {
        private bool pararMovimiento = false;
        public void Parar()
        {
            pararMovimiento = true;  // Cambiar el flag para detener el movimiento
        }
        // Movimiento
        public void _Mover(string direccion, int distancia, Action<object> f = null, object param = null)
        {
            ...
            {
                // Bucle que monitorea la variable choque mientras se mueve
                while (!pararMovimiento)  // Mientras no se haya marcado para parar
                {
                    Thread.Sleep(100);  // Comprobamos cada 100ms
                }

                // Si hubo un choque o se quiere parar el movimiento, detener el movimiento inmediatamente
                if (pararMovimiento)
                {
                    Console.WriteLine("Movimiento detenido por la solicitud de paro.");
                    this._Mover("Stop", 0);  // Detener el dron
                }
        ...
        private void _IrAlPuntoNED(float north, float east, float down, Action<object> callback = null, object param = null)
        {
            try
            {
                if (messageHandler == null)
                    throw new InvalidOperationException("messageHandler es null");
                ...
                byte[] packet = mavlink.GenerateMAVLinkPacket10(
                    MAVLink.MAVLINK_MSG_ID.SET_POSITION_TARGET_LOCAL_NED,
                    msg
                );
                ...
                messageHandler.WaitForMessageBlock(
                    ((int)MAVLink.MAVLINK_MSG_ID.LOCAL_POSITION_NED).ToString(),
                    condition: (m, p) =>
                    {
                        try
                        {
                            return ComprobarParado(m);
                        }
                        catch
                        {
                            return false;
    ...
    object param = null)
        {
            Task.Run(() => _IrAlPuntoNED(north, east, down, callback, param));
        }
